#!/bin/bash
# Simple brutal sh-scripts chain.
if [ "$(echo $PATH | grep -o /usr/local/ps2/bin)" != "/usr/local/ps2/bin" ]
then
 echo -en "\033[36;1m There is no "/usr/local/ps2/bin" was added to \$PATH . Adding... \033[0m\n"
 export PATH=/usr/local/ps2/bin:$PATH
 export GAWK_NO_RE_INTERVALS=1
fi

cd ../sources
if [ ! -e texinfo-6.3.tar.xz ]
then
wget https://ftp.gnu.org/gnu/texinfo/texinfo-6.3.tar.xz || exit -1
fi
cd ../building
tar -Jxvf ../sources/texinfo-6.3.tar.xz
cd texinfo-6.3

autoreconf -fiv || exit -1
sed -i 's/ncurses curses termlib/ncurses ncursesw curses termlib/g' configure
./configure --prefix=/usr/local/ps2/mipsr5900el-unknown-linux-gnu --host=mipsr5900el-unknown-linux-gnu || exit -1

sed -i 's/^CFLAGS =.*/& -std=gnu89/' tools/info/Makefile || exit -1

make -j 4 || exit -1
make install || exit -1

cp -v tools/info/makedoc /usr/bin/

sleep 2

cd ../../scripts
sh 28_readline.sh
