...
cd ../sources
if [ ! -e glibc-2.34.tar.xz ]
then
wget https://ftp.gnu.org/gnu/glibc/glibc-2.34.tar.xz || exit -1
fi
cd ../building
tar -Jxvf ../sources/glibc-2.34.tar.xz
cd glibc-2.34
sed -i 's/^others: \$(common-objpfx)testrun.sh.*/#&\nothers:/' Makefile # avoid tests
patch -p1 < ../../patches/5_glibc-2.34_ACAS.patch
# - that's how it is possible to utilize the mips_atomic_cas part of the sys_sysmips syscall.
# My balls are too small to rewrite GCC's entire libatomic... but big enough to surgically 
# rewrite just its 4 functions needed for just glibc :p
mkdir build
cd build
BUILD_MACHINE=$(gcc -dumpmachine)
echo "libc_cv_forced_unwind=yes" > config.cache
echo "libc_cv_c_cleanup=yes" >> config.cache
# A non-standard FPU remains an issue. So using --without-fp
../configure mipsel-linux --prefix=/usr/local/ps2/mipsr5900el-unknown-linux-gnu --build=$BUILD_MACHINE --host=mipsr5900el-unknown-linux-gnu --enable-shared --with-headers=/usr/local/ps2/mipsr5900el-unknown-linux-gnu/include --enable-kernel=5.4.221 --enable-add-ons --with-tls --with-__thread --without-fp --disable-experimental-malloc --with-pkgversion="GNU libc v2.34, atomics over syscall sys_sysmips (branch MIPS_ATOMIC_CAS) edition" --cache-file=config.cache || exit -1
