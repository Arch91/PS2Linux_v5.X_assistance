...
cd $KERNEL_DIR
rm -rfv usr/initramfs_data.{o,cpio.xz}; make ARCH=mips CROSS_COMPILE=mipsr5900el-unknown-linux-gnu- -j 4 vmlinuz || exit -1
...
#echo -en "\033[36;1mCongratulations! You have successfully built PS2Linux kernel v5.4 in it's test variant. Now, you can proudly move PS2Linux5.4.221.ELF file to the USB flash stick, connect that stick to your PS2 and launch that file using uLaunchELF. Prepare your USB keyboard...\033[0m\n"
