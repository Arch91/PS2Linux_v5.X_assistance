...
patch -p1 < ../../patches/3_glibc-2.13_locale_t_compat.patch
# Zanella's workaround patch for saving/restoring machine *states* for the single-float targets.
patch -p1 < ../../patches/3_glibc-2.13-Zanella-hardsfp-setjmp.patch
mkdir build
cd build
...
# Difference in flags - --with-fp
../configure mipsel-linux --prefix=/usr/local/ps2/mipsr5900el-unknown-linux-gnu --build=i686-pc-linux-gnu --host=mipsr5900el-unknown-linux-gnu --enable-shared --with-headers=/usr/local/ps2/mipsr5900el-unknown-linux-gnu/include --enable-add-ons --with-tls --with-__thread --with-fp --cache-file=config.cache || exit -1
