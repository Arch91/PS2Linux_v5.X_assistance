...
cd ../sources
# Another version. Since that version it is using sqrt function from the appropriate gcc builtin.
if [ ! -e glibc-2.32.tar.xz ]
then
wget https://ftp.gnu.org/gnu/glibc/glibc-2.32.tar.xz || exit -1
fi
cd ../building
tar -Jxvf ../sources/glibc-2.32.tar.xz
cd glibc-2.32
# Zanella's workaround patch for saving/restoring machine *states* for the single-float targets, and also some compatibility fixes for those targets.
patch -p1 < ../../patches/3_glibc-2.32-Zanella-hardsfp-setjmp.patch
mkdir build
cd build
echo "libc_cv_forced_unwind=yes" > config.cache
echo "libc_cv_c_cleanup=yes" >> config.cache
# FPU is not optimized for nextgen PS2Linux yet. For testing purposes only, you have been warned.
# Difference in flags - --with-fp and --disable-werror
../configure mipsel-linux --prefix=/usr/local/ps2/mipsr5900el-unknown-linux-gnu --build=i686-pc-linux-gnu --host=mipsr5900el-unknown-linux-gnu --enable-shared --with-headers=/usr/local/ps2/mipsr5900el-unknown-linux-gnu/include --enable-add-ons --with-tls --with-__thread --with-fp --disable-experimental-malloc --disable-werror --with-pkgversion="GNU libc v2.32, nothing special" --cache-file=config.cache || exit -1
